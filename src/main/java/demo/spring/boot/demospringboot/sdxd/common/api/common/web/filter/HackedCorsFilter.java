 ...
 */
public final class HackedCorsFilter implements Filter {

    private static final Logger log = LoggerFactory.getLogger(HackedCorsFilter.class);
    ...
    public void doFilter(final ServletRequest servletRequest,
        ...
        HackedCorsFilter.CORSRequestType requestType = checkRequestType(request);

        // Adds CORS specific attributes to request.
        if (decorateRequest) {
            HackedCorsFilter.decorateCORSProperties(request, requestType);
        }
        ...
        if (log.isTraceEnabled()) {
            ...
            log.trace("Handle CORS request type: {}:{} - {}, headers: {}", request.getPathInfo(), request.getMethod(), requestType, builder);
        }
        switch (requestType) {
            case SIMPLE:
    ...
    public void init(final FilterConfig filterConfig) throws ServletException {
        ...
        log.trace("Any origin allowed: {}", anyOriginAllowed);
    }
    ...
    protected void handleSimpleCORS(final HttpServletRequest request,
        ...
        HackedCorsFilter.CORSRequestType requestType = checkRequestType(request);
        if (!(requestType == HackedCorsFilter.CORSRequestType.SIMPLE ||
                requestType == HackedCorsFilter.CORSRequestType.ACTUAL)) {
            throw new IllegalArgumentException(
                    sm.getString("corsFilter.wrongType2",
                            HackedCorsFilter.CORSRequestType.SIMPLE,
                            HackedCorsFilter.CORSRequestType.ACTUAL));
        }
        ...
        if (!isOriginAllowed(origin)) {
            ...
            log.trace("Origin {} not allowed.", origin);
            return;
        }

        if (!allowedHttpMethods.contains(method)) {
            handleInvalidCORS(request, response, filterChain);
            log.trace("Method {} not allowed.", method);
            return;
        }
        ...
        // Forward the request down the security chain.
    ...
    protected void handlePreflightCORS(final HttpServletRequest request,
        ...
        if (accessControlRequestMethod == null) {
            handleInvalidCORS(request, response, filterChain);
            log.trace("Access control request method is null.");
            return;
        } else {
        ...
        if (!allowedHttpMethods.contains(accessControlRequestMethod)) {
            handleInvalidCORS(request, response, filterChain);
            log.trace("Method {} not allowed.", accessControlRequestMethod);
            return;
        }
        ...
        if (!accessControlRequestHeaders.isEmpty()) {
            for (String header : accessControlRequestHeaders) {
                if (!allowedHttpHeaders.contains(header)) {
                    handleInvalidCORS(request, response, filterChain);
                    log.trace("Header {} not allowed.", header);
                    return;
                }
            }
        }
        ...
        // Do not forward the request down the security chain.
    }
     ...
     * request down the security chain.
     *
     * @param request     The {@link HttpServletRequest} object.
     * @param response    The {@link HttpServletResponse} object.
     * @param filterChain The {@link FilterChain} object.
